...
const distortions = {
    mountainDistortion: {
        ...
        getJS: (progress, time) => {
            const fix = 0.02;
            const f = mountainUniforms.uFreq.value, a = mountainUniforms.uAmp.value;
    ...
    xyDistortion: {
        ...
        getJS: (progress, time) => {
            const fix = 0.02;
            const f = xyUniforms.uFreq.value, a = xyUniforms.uAmp.value;
            const d = new THREE.Vector3(
                Math.cos(progress*Math.PI*f.x+time)*a.x - Math.cos(fix*Math.PI*f.x+time)*a.x,
    ...
    LongRaceDistortion: {
        ...
        getJS: (progress, time) => {
            const cam = 0.0125;
            const f = LongRaceUniforms.uFreq.value, a = LongRaceUniforms.uAmp.value;
            const d = new THREE.Vector3(
    ...
    deepDistortion: {
        ...
        getJS: (progress, time) => {
            ...
            const getX = q => Math.sin(q*Math.PI*f.x+time)*a.x;
            const getY = q => Math.pow(q*p.x,p.y) + Math.sin(q*Math.PI*f.y+time)*a.y;
...
// ── Default fallback distortion (used when no named distortion is found) ──────
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const options = {
        ...
        colors: { ...DEFAULT_EFFECT_OPTIONS.colors }
    };
