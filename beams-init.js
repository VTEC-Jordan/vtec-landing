// Beams hero background — Three.js ES module port
// Faithfully ported from React Bits Beams component (no React/R3F required)

import * as THREE from 'three';

// ── Config (matching React Bits Beams defaults) ──────────────
const BEAM_WIDTH      = 2;
const BEAM_HEIGHT     = 15;
const LIGHT_COLOR     = '#00C8E0';  // bright site-teal (shows through the overlay)
const SPEED           = 2;
const NOISE_INTENSITY = 1.75;
const SCALE           = 0.2;
const ROTATION        = 0;          // degrees

// ── Classic 3-D Perlin noise — identical GLSL to the original ──
const noiseSrc = /* glsl */`
...
// ── extendMaterial — same logic as the React component ──────────
...
// ── Geometry — identical to createStackedPlanesBufferGeometry ───
function createStackedPlanesGeometry(n, width, height, spacing, heightSegments) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    for (let j = 0; j <= heightSegments; j++) {
      ...
      if (j < heightSegments) {
        const a = vertexOffset, b = vertexOffset + 1,
              c = vertexOffset + 2, d = vertexOffset + 3;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const beamNumber = isMobile ? 6 : 12;
  ...
  // ── Camera — <PerspectiveCamera position={[0,0,20]} fov={30}> ─
  ...
  // ── Lights — <ambientLight intensity={1}/> + <DirLight/> ──────
...
gl_FragColor.rgb -= randomNoise / 15. * uNoiseIntensity;`
    ...
    material: { fog: true },
    uniforms: {
      // Black planes — directional light + specular creates the colored beam look
  ...
  // ── Mesh ──────────────────────────────────────────────────
  const geometry = createStackedPlanesGeometry(beamNumber, BEAM_WIDTH, BEAM_HEIGHT, 0, 100);
  const mesh     = new THREE.Mesh(geometry, beamMaterial);

  // ── Group — <group rotation={[0, 0, degToRad(rotation)]}> ─────
  ...
  // ── Render loop ───────────────────────────────────────────
  // Pause when tab is hidden — saves CPU/GPU on low-end devices
  let paused = false;
  ...
  function animate() {
    ...
    if (paused) return;
    // Matches: mesh.current.material.uniforms.time.value += 0.1 * delta
